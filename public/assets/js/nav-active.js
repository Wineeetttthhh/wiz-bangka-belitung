    ...
    function initActiveNavigation() {
        ...
        function setActive(targetLink) {
            navLinks.forEach(link => {
                ...
                if (isThisTarget) {
                    link.classList.add('text-[#008444]', 'font-bold', 'border-[#008444]');
                    link.classList.remove('text-slate-700', 'text-on-surface-variant', 'border-transparent');
                } else {
                    link.classList.remove('text-[#008444]', 'font-bold', 'border-[#008444]');
        ...
        function activateByHref(targetHref) {
            ...
            navLinks.forEach(link => {
                ...
                if (href === targetHref || href === `/${targetHref}` || href === `index.html${targetHref}`) {
                    matchedLink = link;
                }
            });

            // Fallback for home page
            if (!matchedLink && (targetHref === '/' || targetHref === 'index.html' || targetHref === '')) {
                navLinks.forEach(link => {
                    const href = link.getAttribute('href');
                    if (href === '/' || href === 'index.html' || href === '#' || href === 'index.html#') {
                        matchedLink = link;
                    }
                });
            }
        ...
        function updateActiveOnPageLoad() {
            ...
            if (isIndexPage || fullPath === '/' || fullPath === '') {
            ...
            } else if (fullPath.startsWith('/program') || path === 'program.html') {
                activateByHref('/program');
            } else if (fullPath.startsWith('/laporan') || path === 'laporan.html') {
                activateByHref('/laporan');
            } else if (fullPath.startsWith('/berita') || path === 'berita.html') {
                activateByHref('/berita');
            }
        }
