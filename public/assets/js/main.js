...
function initNavigation() {
    ...
    navLinks.forEach(link => {
        link.addEventListener('click', () => {
            ...
            } else if (mainNav && !mainNav.classList.contains('hidden')) {
        ...
        if (activeBanks.length === 0) {
            ...
            siteBankContainer.className = activeBanks.length === 1 
                ...
                : activeBanks.length === 2 
                    ? "max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6 w-full" 
                    : "max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full";
            ...
            siteBankContainer.innerHTML = activeBanks.map(b => {
                ...
                return `
                <div class="bg-gradient-to-br from-emerald-50 to-teal-50 p-6 sm:p-8 rounded-3xl border-2 border-emerald-500/40 shadow-md space-y-4 flex flex-col justify-between">
                    <div>
                        ...
                        <h3 class="text-xl font-bold text-emerald-800">${b.bank}</h3>
                        <p class="text-sm text-slate-600 mt-0.5">Saluran Resmi Donasi Zakat, Infak &amp; Sedekah</p>
                        <div class="mt-4 font-mono text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-widest bg-white/90 py-3 px-4 rounded-xl border border-emerald-200/80 select-all text-center shadow-xs">
                            ${rawNum}
                        </div>
                        <p class="text-sm text-slate-600 font-semibold mt-2 text-center">A.n. <strong class="text-emerald-800">${b.holder || 'WIZ Bangka Belitung'}</strong></p>
                    </div>
                    <button onclick="copyToClipboard('${rawNum}', 'No. Rekening ${b.bank} (${rawNum}) berhasil disalin!')" class="mt-2 w-full bg-emerald-600 hover:bg-emerald-700 text-white py-3 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2 shadow-sm cursor-pointer">
